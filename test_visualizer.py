...
class TEST_DATA_CAPTIONS(db.Model):
    ...
    bert_base_score = db.Column(db.Float)
...
def tables(page_num=1):
    """
        Controller File
    """
    articles = TEST_DATA_CAPTIONS.query.order_by(TEST_DATA_CAPTIONS.bert_base_score).paginate(per_page=100, page=page_num, error_out=True)
