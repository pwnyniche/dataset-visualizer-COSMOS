...
from db_cursor import mydb, my_cursor
...
def create_tables():
    ...
    my_cursor.execute("create table TEST_DATA_CAPTIONS ( id int auto_increment primary key, caption1 varchar(5000) null, caption2 varchar(5000) null, context_label  int null, base_url varchar(1000) null, article_url varchar(2000) null, img_local_path varchar(1000) null, bert_base_score varchar(1000) null  )")
...
def insert_test_data(data_list, table_name):
    ...
    for data in data_list:
        sql_insert = 'INSERT INTO ' + table_name + ' (caption1, caption2, context_label, base_url, article_url, img_local_path, bert_base_score ) VALUES (%s, %s, %s, %s, %s, %s, %s)'
        if "article_url" in data.keys() and len(data["article_url"]) > 0:
            base_url = "{0.netloc}".format(urlsplit(data['article_url'])).replace("www.", "")
        else:
            data["article_url"] = ""
            base_url = ""
        data_tuple = (data["caption1"], data["caption2"], data["context_label"], base_url, data["article_url"], data["img_local_path"],  data["bert_base_score"])
...
def insert_into_tables():
    ...
    articles = read_json_data(os.path.join(DATA_DIR, "annotations", "test_false_cases.json"))
